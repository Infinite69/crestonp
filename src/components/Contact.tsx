...
import { Phone, MapPin, Instagram, Loader2, CheckCircle2, MessageCircle } from 'lucide-react';
...
export default function Contact() {
  ...
  const whatsappUrl = `https://wa.me/${phoneNumber}?text=Hello! I'm interested in a design consultation.`;
        ...
        <div className="grid lg:grid-cols-2 gap-20">
          <div className="space-y-12">
            ...
            <div className="space-y-6">
              <Link href={`tel:+91${phoneNumber}`} className="flex items-center space-x-4 group">
              ...
              <Link href={whatsappUrl} target="_blank" className="flex items-center space-x-4 group">
                ...
                <div>
                  ...
                  <div className="font-bold text-lg">Chat with a Designer</div>
                </div>
              </Link>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                <div className="grid md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <Input placeholder="John Doe" required className="bg-background rounded-none" />
                  </div>
                  <div className="space-y-2">
                    <label className="text-sm font-medium">Email Address</label>
                    <Input type="email" placeholder="john@example.com" required className="bg-background rounded-none" />
                  </div>
                </div>
                <div className="space-y-2">
                  <label className="text-sm font-medium">Phone Number</label>
                  <Input placeholder="+91 00000 00000" required className="bg-background rounded-none" />
                </div>
                <div className="space-y-2">
                  <label className="text-sm font-medium">Your Message</label>
                  <Textarea placeholder="How can we help you?" required className="bg-background min-h-[150px] rounded-none" />
                </div>
                <Button 
                  type="submit" 
                  disabled={isSubmitting} 
                  className="w-full h-14 bg-primary text-white text-lg font-bold rounded-none"
                >
                  {isSubmitting ? (
                    <>
